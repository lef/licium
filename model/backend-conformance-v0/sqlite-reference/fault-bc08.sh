...
[ "$#" -eq 7 ] || {
    echo "usage: fault-bc08.sh DB VERB HOOK PHASE NONCE REVISION EFFECT" >&2
    exit 2
}
...
case "$hook:$phase" in
    hook-bc08-after-observation:after-observation|\
    hook-bc08-after-transition:after-transition|\
    hook-bc08-after-view-header:after-view-header|\
    hook-bc08-after-view-row:after-view-row|\
    hook-bc08-before-current-update:before-current-update) ;;
    *) exit 2 ;;
esac
...
")
        [ "$changed" = 1 ] || {
            echo BC08_FAULT_CLEAR_MISSING >&2
            exit 1
        }
