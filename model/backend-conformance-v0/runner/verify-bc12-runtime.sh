...
[ "$#" -eq 5 ] || {
    echo "usage: verify-bc12-runtime.sh ARTIFACT_DIR RUN NS ASSERTION SCENARIO" >&2
    exit 2
}
...
[ -d "$artifact_dir" ] && [ ! -L "$artifact_dir" ] ||
    fail BC12_ARTIFACT_SET_INVALID
...
do
    ...
    [ ! -L "$file" ] && [ "$(stat -c '%a' "$file")" = 644 ] ||
        fail BC12_ARTIFACT_MODE_INVALID
...
) || fail BC12_CASE_COVERAGE_INVALID
surface=${case_row%%	*}
operation=${case_row#*	}

case "$assertion" in
    BC12_ARCHIVE_BYPASS|BC12_CANONICAL_UNCHANGED) rows=5 ;;
    BC12_DECISION_PROVENANCE) rows=10 ;;
    BC12_DERIVED_PROTECTION) rows=8 ;;
    BC12_ELIGIBILITY_DELETE) rows=4 ;;
    BC12_FORGET_BYPASS|BC12_FORGET_CONSUMED) rows=5 ;;
    BC12_NOOP_EVALUATOR) rows=6 ;;
    BC12_PLACEMENT_DECISION) rows=8 ;;
    BC12_PROTECTION_BYPASS) rows=13 ;;
    BC12_WINDOW_BYPASS) rows=7 ;;
    *) fail BC12_CASE_COVERAGE_INVALID ;;
esac
...
check_shape action-receipts.tsv 12 1 BC12_SUT_ACTION_MISSING
check_shape raw-seal.tsv 9 1 BC12_RAW_SEAL_INVALID
...
' "$artifact_dir/raw-seal.tsv" || fail BC12_RAW_SEAL_INVALID
...
' "$artifact_dir/raw-observations.tsv" \
    ...
    "$artifact_dir/coverage.tsv" || fail BC12_COVERAGE_INVALID

check_shape command-receipts.tsv 12 6 BC12_COMMAND_CUSTODY_INVALID
check_shape coverage.tsv 6 "$rows" BC12_COVERAGE_INVALID
check_shape exclusions.tsv 6 0 BC12_EXCLUSION_INVALID
check_shape fault-markers.tsv 6 0 BC12_FAULT_EVIDENCE_INVALID
check_shape normalized-observations.tsv 6 "$rows" \
    BC12_NORMALIZED_CONTRACT_INVALID
check_shape oracle-result.tsv 6 1 BC12_ORACLE_INVALID
check_shape pragma.tsv 6 5 BC12_PRAGMA_EVIDENCE_INVALID
check_shape raw-observations.tsv 6 "$rows" BC12_RAW_CONTRACT_INVALID
...
awk -F '	' -v run="$run" -v ns="$namespace" -v scenario="$scenario" \
    ...
    $1 != run || $2 != ns || $3 != scenario || $4 != assertion ||
        ...
        $8 != "evaluation-1" || $9 != "root-set-1" ||
        $10 != "policy-input-1" || $11 != "accepted" ||
...
' "$artifact_dir/command-receipts.tsv" ||
    fail BC12_COMMAND_CUSTODY_INVALID
...
' "$artifact_dir/pragma.tsv" || fail BC12_PRAGMA_EVIDENCE_INVALID

"$script_dir/normalize-bc12.sh" "$artifact_dir/raw-observations.tsv" \
    "$scenario" >"$tmp/normalized" ||
    fail BC12_NORMALIZED_CONTRACT_INVALID
