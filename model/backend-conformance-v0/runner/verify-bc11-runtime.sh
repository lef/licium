...
[ "$#" -eq 5 ] || {
    echo "usage: verify-bc11-runtime.sh ARTIFACT_DIR RUN NS ASSERTION SCENARIO" >&2
    exit 2
}
...
[ -d "$artifact_dir" ] && [ ! -L "$artifact_dir" ] ||
    fail BC11_ARTIFACT_SET_INVALID
...
do
    ...
    [ ! -L "$file" ] && [ "$(stat -c '%a' "$file")" = 644 ] ||
        fail BC11_ARTIFACT_MODE_INVALID
...
) || fail BC11_CASE_COVERAGE_INVALID
surface=${case_row%%	*}
operation=${case_row#*	}

case "$assertion" in
    BC11_EXPLANATION_CLOSURE) rows=6 ;;
    BC11_FINDING_CROSS_LINK|BC11_FINDING_DANGLING) rows=6 ;;
    BC11_LATEST_SUBSTITUTION) rows=11 ;;
    BC11_MISSING_AS_EMPTY) rows=35 ;;
    BC11_REPLAY_RESULT) rows=10 ;;
    BC11_SILENT_CROSS_LINK|BC11_SILENT_DANGLING) rows=9 ;;
    *) fail BC11_CASE_COVERAGE_INVALID ;;
esac
...
check_shape action-receipts.tsv 12 1 BC11_SUT_ACTION_MISSING
check_shape raw-seal.tsv 9 1 BC11_RAW_SEAL_INVALID
...
' "$artifact_dir/raw-seal.tsv" || fail BC11_RAW_SEAL_INVALID
...
' "$artifact_dir/raw-observations.tsv" \
    ...
    "$artifact_dir/coverage.tsv" || fail BC11_COVERAGE_INVALID

check_shape command-receipts.tsv 12 6 BC11_COMMAND_CUSTODY_INVALID
check_shape coverage.tsv 6 "$rows" BC11_COVERAGE_INVALID
check_shape exclusions.tsv 6 0 BC11_EXCLUSION_INVALID
check_shape fault-markers.tsv 6 0 BC11_FAULT_EVIDENCE_INVALID
check_shape normalized-observations.tsv 6 "$rows" \
    BC11_NORMALIZED_CONTRACT_INVALID
check_shape oracle-result.tsv 6 1 BC11_ORACLE_INVALID
check_shape pragma.tsv 6 5 BC11_PRAGMA_EVIDENCE_INVALID
check_shape raw-observations.tsv 6 "$rows" BC11_RAW_CONTRACT_INVALID
...
' "$artifact_dir/command-receipts.tsv" ||
    fail BC11_COMMAND_CUSTODY_INVALID
...
' "$artifact_dir/pragma.tsv" || fail BC11_PRAGMA_EVIDENCE_INVALID

"$script_dir/normalize-bc11.sh" "$artifact_dir/raw-observations.tsv" \
    "$scenario" >"$tmp/normalized" ||
    fail BC11_NORMALIZED_CONTRACT_INVALID
