...
[ "$#" -eq 2 ] || {
    echo "usage: materialize-bc06-session-controls.sh SESSION_DIR OUTPUT" >&2
    exit 2
}
...
record()
{
    ...
    case "$kind" in
        copied)
            ...
            evidence="$case_dir/run-b/bc06-pure-zero-axes/action-receipts.tsv"
            ;;
        drift)
    ...
    [ "$status" -ne 0 ] && [ "$observed" = "$target" ] || {
        echo BC06_SESSION_CONTROL_EXECUTION_INVALID >&2
        exit 1
    }
...
do
    case "$id" in
        harness-bc06-copied-run)
            record "$id" "$class" "$mutation" "$target" copied
            ;;
        harness-bc06-second-run-drift)
