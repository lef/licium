...
runtime_verifier="$script_dir/verify-bc10-runtime.sh"
...
[ -d "$run_a" ] && [ ! -L "$run_a" ] &&
    [ -d "$run_b" ] && [ ! -L "$run_b" ] ||
    fail BC10_SESSION_ARTIFACT_MISSING
...
do
    ...
    awk -F '	' -v run=run-a -v ns="$ns_a" \
        ...
        END { if (NR != 1) exit 1 }
    ' "$dir_a/action-receipts.tsv" || fail BC10_COPIED_RUN_DETECTED
    awk -F '	' -v run=run-b -v ns="$ns_b" \
        -v scenario="$scenario" -v assertion="$assertion" '
        NF != 12 || $1 != run || $2 != ns || $3 != scenario ||
            $4 != assertion || $12 != "action-" run { exit 1 }
        END { if (NR != 1) exit 1 }
    ' "$dir_b/action-receipts.tsv" || fail BC10_COPIED_RUN_DETECTED

    nonce_a=$(awk -F '	' '{ print $12 }' "$dir_a/action-receipts.tsv")
    nonce_b=$(awk -F '	' '{ print $12 }' "$dir_b/action-receipts.tsv")
    [ "$nonce_a" != "$nonce_b" ] || fail BC10_COPIED_RUN_DETECTED
...
done <"$base_dir/bc10-scenario-ids.tsv"

cmp -s "$tmp/canonical-a" "$tmp/canonical-b" ||
    fail BC10_SECOND_RUN_DRIFT_DETECTED
...
do
    expected_count=$(wc -l <"$base_dir/bc10-scenario-ids.tsv" | tr -d ' ')
    [ ! -L "$session_dir/$run/runtime-status.tsv" ] &&
        [ "$(stat -c '%a' "$session_dir/$run/runtime-status.tsv")" = 644 ] ||
        fail BC10_SESSION_ARTIFACT_INVALID
    awk -F '	' -v expected_count="$expected_count" '
        NF != 2 || $2 != "BC10_RUNTIME_VALID" || seen[$1]++ { exit 1 }
